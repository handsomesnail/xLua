 ...
 *may obtain a copy of the License at http://opensource.org/licenses/MIT Unless required by applicable law or agreed to
...
LUA_API void *xlua_tag() { return &tag; }

LUA_API int xlua_get_registry_index() { return LUA_REGISTRYINDEX; }

LUA_API int xlua_get_lib_version() { return 105; }
...
#undef lua_setglobal
LUA_API void lua_setglobal(lua_State *L, const char *name) { lua_setfield(L, LUA_GLOBALSINDEX, name); }

LUA_API int lua_isinteger(lua_State *L, int idx) { return 0; }

LUA_API uint32_t xlua_objlen(lua_State *L, int idx) { return lua_objlen(L, idx); }

LUA_API uint32_t xlua_touint(lua_State *L, int idx) { return (uint32_t)lua_tonumber(L, idx); }

LUA_API void xlua_pushuint(lua_State *L, uint32_t n) { lua_pushnumber(L, n); }
#endif
...
LUA_API uint32_t xlua_objlen(lua_State *L, int idx) { return (uint32_t)luaL_len(L, idx); }
...
LUA_API void xlua_pushuint(lua_State *L, uint32_t n) { lua_pushinteger(L, n); }

#undef lua_insert
LUA_API void lua_insert(lua_State *L, int idx) { lua_rotate(L, idx, 1); }
...
#undef lua_tonumber
LUA_API lua_Number lua_tonumber(lua_State *L, int idx) { return lua_tonumberx(L, idx, NULL); }
...
LUA_API void xlua_rawgeti(lua_State *L, int idx, int64_t n) { lua_rawgeti(L, idx, (lua_Integer)n); }

LUA_API void xlua_rawseti(lua_State *L, int idx, int64_t n) { lua_rawseti(L, idx, (lua_Integer)n); }
...
LUA_API int xlua_tointeger(lua_State *L, int idx) { return (int)lua_tointeger(L, idx); }

LUA_API void xlua_pushinteger(lua_State *L, int n) { lua_pushinteger(L, n); }

LUA_API void xlua_pushlstring(lua_State *L, const char *s, int len) { lua_pushlstring(L, s, len); }

LUALIB_API int xluaL_loadbuffer(lua_State *L, const char *buff, int size, const char *name) {
...
LUALIB_API int xlua_upvalueindex(int n) { return lua_upvalueindex(2 + n); }
...
LUALIB_API int xlua_unpack_float6(void *p, int offset, float *f1, float *f2, float *f3, float *f4, float *f5,
                                  float *f6) {
  CSharpStruct *css = (CSharpStruct *)p;
  if (css->fake_id != -1 || css->len < offset + sizeof(float) * 6) {
    return 0;
...
static const lua_CFunction direct_getters[10] = {
    xlua_struct_get_int8_t,  xlua_struct_get_uint8_t,  xlua_struct_get_int16_t, xlua_struct_get_uint16_t,
    xlua_struct_get_int32_t, xlua_struct_get_uint32_t, xlua_struct_get_int64_t, xlua_struct_get_uint64_t,
    xlua_struct_get_float,   xlua_struct_get_double};

static const lua_CFunction direct_setters[10] = {
    xlua_struct_set_int8_t,  xlua_struct_set_uint8_t,  xlua_struct_set_int16_t, xlua_struct_set_uint16_t,
    xlua_struct_set_int32_t, xlua_struct_set_uint32_t, xlua_struct_set_int64_t, xlua_struct_set_uint64_t,
    xlua_struct_set_float,   xlua_struct_set_double};

int nop(lua_State *L) { return 0; }
...
LUA_API void *xlua_gl(lua_State *L) { return G(L); }
